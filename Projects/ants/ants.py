...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if self.can_contain(place.ant):
                place.ant = self
            elif not place.ant.can_contain(self):
    ...
    def buff(self):
        ...
        if not self.doubled:
            self.damage *= 2
            self.doubled = True
        if self.is_container and self.ant_contained != None and not self.ant_contained.doubled:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        def f(place, n):
            if place.is_hive:
                return [None, 0]
            elif place.bees != [] and self.min_range <= n <= self.max_range:
                return [random_bee(place.bees), n]
            return f(place.entrance, n + 1)
        return f(self.place, 0)[0]
        # END Problem 3 and 4
    ...
    def reduce_health(self, amount):
        ...
        def f(a):
            ...
            while i < len(self.place.bees):
                if self.place.bees[i].health - a <= 0:
    ...
    def action(self, gamestate):
        ...
        while place != None:
            if place.ant != None:
                place.ant.buff()
            place = place.exit
        super().throw_at(self.nearest_bee())
        # END Problem 12
...
class Bee(Insect):
    ...
    name = 'Bee'
    damage = 1
    scaring = scared = False
    is_waterproof = True
    scare_duration = slow_duration = 0
    # OVERRIDE CLASS ATTRIBUTES HERE
    ...
    def action(self, gamestate):
        ...
        if self.scaring:
            ...
            self.scare_duration -= 1
            if self.scare_duration == 0:
                if self.slow_duration and gamestate.time % 2:
                    self.scare_duration += 1
                else:
                    self.scared = True
                    self.scaring = False
    ...
    def scare(self, length):
        ...
        """
        # BEGIN Problem EC
        if not self.scaring and not self.scared:
            self.scaring = True
            self.scare_duration += length
        # END Problem EC
