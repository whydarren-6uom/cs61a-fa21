...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    if not len(reference_words) or not len(typed_words):
        if len(typed_words) == len(reference_words):
            return 100.0
        return 0.0
    def f(sum, i):
        if i == len(typed_words) or i == len(reference_words):
            return sum
        elif typed_words[i] == reference_words[i]:
            return f(sum + 1, i + 1)
        return f(sum, i + 1)
    return f(0, 0) / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    """
    # BEGIN PROBLEM 5
    minx, minf = diff_function(typed_word, valid_words[0], limit), valid_words[0]
    for word in valid_words:
        if typed_word == word:
            return word
        elif diff_function(typed_word, word, limit) < minx:
            minx = diff_function(typed_word, word, limit)
            minf = word
    if minx > limit:
        return typed_word
    return minf
...
def feline_flips(start, goal, limit):
    ...
    def f(i, sum):
        if sum > limit:
            ...
            return sum + abs(len(start) - len(goal))
        elif start[i] == goal[i]:
            return f(i - 1, sum)
        return f(i - 1, sum + 1)
    return f(min(len(start) - 1, len(goal) - 1), 0)
    # END PROBLEM 6
...
def minimum_mewtations(start, goal, limit):
    ...
    def f(start, goal, i):
        ...
        elif start == '' or goal == '' or start == [] or goal == []:
            return abs(len(goal) - len(start))

        elif start[0] == goal[0]:
            return f(start[1:], goal[1:], i)

        else:
            add = 1 + f(start, goal[1:], i + 1)
            remove = 1 + f(start[1:], goal, i + 1)
            substitute = 1 + f(start[1:], goal[1:], i + 1)
...
def final_diff(start, goal, limit):
    ...
    def f(start, goal, i):
        ...
        elif start[0] == goal[1] and start[1] == goal[0]:
            return 1 + f(start[2:], goal[2:], i + 1)
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    while i < len(sofar):
        if sofar[i] == prompt[i]:
            ...
            break
    return upload({'id' : user_id, 'progress' : sum / len(prompt)}) or sum / len(prompt)
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    while i < len(times_per_player):
        lstn, j = [], 1
        while j < len(times_per_player[i]):
            lstn.append(times_per_player[i][j] - times_per_player[i][j - 1])
            j += 1
        lst.append(lstn)
        i += 1
    ...
    for i in player_indices:
        ...
        for j in word_indices:
            ...
            for n in player_indices:
                if time(match, i, j) > time(match, n, j) or (time(match, i, j) == time(match, n, j) and i > n):
                    judge = 0
            if judge == 1:
                lstn.append(word_at(match, j))
        lst.append(lstn)
    return lst
    # END PROBLEM 10
