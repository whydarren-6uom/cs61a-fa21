class VendingMachine:
    ...
    money = stock = 0
    plz_restock = 'Nothing left to vend. Please restock.'
    hereis = 'Here is your '
    ...
    def vend(self):
        if not self.stock:
            ...
            self.money = 0
            self.stock -= 1
            return f'{self.hereis}' + self.name + ' and $' + str(remain) + ' change.'
        self.stock -= 1
        return f'{self.hereis}' + self.name + '.'
    
    def add_funds(self, fund):
        if not self.stock:
            return f'{self.plz_restock} {self.hereis}$' + str(fund) + '.'
        self.money += fund
...
def store_digits(n):
    ...
    def f(n, result=None):
        if not n:
            ...
            return f(n // 10, Link(n % 10))
        return f(n // 10, Link(n % 10, result))
    return f(n)
...
def deep_map_mut(fn, link):
    ...
    def f(fn, link):
        if isinstance(link.first, int):
            ...
            link.first = f(fn, link.first)
        if link.rest != ():
            link.rest = f(fn, link.rest)
        return link
    link = f(fn, link)
...
def two_list(vals, amounts):
    ...
    def f(v, a, r):
        if v == []:
            ...
            a[len(a)-1] -= 1
            return f(v, a, r)
        return f(v[:len(v)-1], a[:len(a)-1], r)
    amounts[len(amounts)-1] -= 1
    return f(vals, amounts, Link(vals[len(vals)-1]))
