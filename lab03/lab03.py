...
def protected_secret(password, secret, num_attempts):
    ...
    def get_secret(password_attempt):
        if num_attempts == 0:
            return print("SECRET LOCKED") or protected_secret(password, secret, 0)
        elif password_attempt == password:
            return print(secret) or protected_secret(password, secret, num_attempts)
        else:
